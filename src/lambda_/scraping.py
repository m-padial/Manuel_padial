...
def scrapeo_opciones_y_futuros():
    ...
    filas_opciones = driver.find_elements(
        By.XPATH,
        '//*[@id="tblOpciones"]/tbody/tr[contains(@class, "text-right")]'
    )
    datos_opciones = []
    ...
    for fila in filas_opciones:
        ...
        tipo_opcion = (
            "Call" if tipo.startswith("OCE")
            else "Put" if tipo.startswith("OPE")
            else None
        )
        celdas = fila.find_elements(By.TAG_NAME, "td")
        strike = (
            celdas[0].get_attribute('innerHTML').strip()
            .replace('.', '').replace(',', '.')
            if celdas else None
        )
        ant = (
            celdas[-1].get_attribute('innerHTML').strip()
            .replace('.', '').replace(',', '.')
            if celdas else None
        )
        ant = ant.replace('&nbsp;', '').strip() if ant else None
        datos_opciones.append([
            datetime.today().date(),
            vencimiento,
            float(strike) if strike and strike != '-' else None,
            tipo_opcion,
            float(ant) if ant and '-' not in ant else None,
            None
        ])
    ...
    filas_futuros = driver.find_elements(
        By.XPATH,
        (
            '//*[@id="Contenido_Contenido_tblFuturos"]'
    ...
    df_futuros = pd.DataFrame(
        datos_futuros,
        columns=['hoy', 'vencimiento', 'ant_futuro']
    )
